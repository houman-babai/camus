...
public class CamusJob extends Configured implements Tool {
    ...
    public void run(Class<? extends InputFormat> inputFormatClass, Class<? extends OutputFormat> outputFormatClass)
            throws Exception {
        ...
        long limit = (long) (content.getQuota() * job.getConfiguration().getFloat(ETL_EXECUTION_HISTORY_MAX_OF_QUOTA,
                (float) .5));
        ...
        if (hadExecutionErrors
                && props.getProperty(ETL_FAIL_ON_ERRORS, Boolean.FALSE.toString()).equalsIgnoreCase(
        ...
        if (EtlInputFormat.reportJobFailureUnableToGetOffsetFromKafka) {
            ...
            throw new RuntimeException(
                    "Some topics skipped due to failure in getting latest offset from Kafka leaders.");
        }
    ...
    private void checkIfTooManySkippedMsg(Counters counters) {
        double maxPercentSkippedSchemaNotFound = Double.parseDouble(props.getProperty(
        ...
        for (String groupName : counters.getGroupNames()) {
            if (groupName.equals(EtlRecordReader.KAFKA_MSG.class.getName())) {
                ...
                for (Counter counter : group) {
                    ...
                    } else if (counter.getDisplayName().equals(
                            EtlRecordReader.KAFKA_MSG.SKIPPED_SCHEMA_NOT_FOUND.toString())) {
    ...
    private void checkIfTooManySkippedMsg(double maxPercentAllowed, long actualSkipped, long actualSuccessful,
        ...
        if (actualSkippedPercent > maxPercentAllowed) {
            String message = "job failed: " + actualSkippedPercent + "% messages skipped due to " + reason
                    + ", maximum allowed is " + maxPercentAllowed + "%";
            log.error(message);
    ...
    private Map<String, List<Pair<EtlKey, ExceptionWritable>>> readErrors(final FileSystem fs,
        ...
        final int maxErrorsFromFile = Integer.parseInt(props.getProperty(ETL_MAX_ERRORS_TO_PRINT_FROM_FILE,
                ETL_MAX_ERRORS_TO_PRINT_FROM_FILE_DEFAULT));
        final Map<String, List<Pair<EtlKey, ExceptionWritable>>> errors = Maps.newHashMap();

        for (final FileStatus f : fs.listStatus(newExecutionOutput,
                new PrefixFilter(EtlMultiOutputFormat.ERRORS_PREFIX))) {
            int errorCounter = 0;
            ...
            final List<Pair<EtlKey, ExceptionWritable>> errorsFromFile = Lists
                    .<Pair<EtlKey, ExceptionWritable>> newArrayList();

            while (reader.next(key, value)) {
                errorCounter++;

                if (errorCounter <= maxErrorsFromFile) {
                    errorsFromFile.add(new Pair<EtlKey, ExceptionWritable>(new EtlKey(key), new ExceptionWritable(value
                            .toString())));
                }
            }
            ...
            if (errorCounter > 0) {
                if (errorCounter > maxErrorsFromFile) {
                    errorsFromFile
                    .add(new Pair<EtlKey, ExceptionWritable>(new EtlKey(key), new ExceptionWritable(
                            "... Too many errors to show. " + "Skipped " + (errorCounter - maxErrorsFromFile)
                            + " ...")));
                }
    ...
    public void sendTrackingCounts(JobContext job, FileSystem fs, Path newExecutionOutput,
            Class<? extends EtlCounts> etlCountsClass) throws IOException, URISyntaxException,
        ...
        if (EtlMultiOutputFormat.isRunTrackingPost(job)) {
            ...
            for (FileStatus gfileStatus : gstatuses) {
                ...
                ArrayList<EtlCounts> countsObjects = mapper.readValue(buffer.toString(),
                ...
                for (EtlCounts count : countsObjects) {
                    ...
                    if (allCounts.containsKey(topic)) {
                        ...
                        existingCounts.setLastTimestamp(Math.max(existingCounts.getLastTimestamp(),
                                count.getLastTimestamp()));
                        existingCounts.setStartTime(Math.min(existingCounts.getStartTime(), count.getStartTime()));
                        existingCounts.setFirstTimestamp(Math.min(existingCounts.getFirstTimestamp(),
            ...
            for (FileStatus countFile : fs.listStatus(newExecutionOutput, new PrefixFilter("counts"))) {
                if (props.getProperty(ETL_KEEP_COUNT_FILES, "false").equals("true")) {
                    fs.rename(countFile.getPath(), new Path(props.getProperty(ETL_COUNTS_PATH), countFile.getPath()
                            .getName()));
                } else {
            ...
            for (EtlCounts finalEtlCounts : allCounts.values()) {
                EtlCounts finalCounts = etlCountsClass.getDeclaredConstructor(EtlCounts.class).newInstance(
                        finalEtlCounts);
    ...
    /**
     * Creates a diagnostic report based on provided logger defaults to TimeLogger which focusses on timing breakdowns. Useful for
     * determining where to optimize.
     *
     * @param job
     * @param timingMap
     * @throws IOException
